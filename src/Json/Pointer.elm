        ...
        , whenArray =
            \array index ->
                insert index v array
                    |> Result.map (Array.toList >> JE.list)
        , whenObject =
            \object field ->
                            ...
                            (\index ->
                                get index array
                                    |> Result.andThen
                                        (\x ->
                                            operation names x
                                                |> Result.map (\v -> Array.set index v array |> Array.toList |> JE.list)
                                        )
                            )
                )
                (\object ->
...
arrayOrObject v ifArray ifObject =
    ...
    of
        ...
        Err _ ->
            (JD.decodeValue (JD.dict JD.value) v |> Result.andThen ifObject)
